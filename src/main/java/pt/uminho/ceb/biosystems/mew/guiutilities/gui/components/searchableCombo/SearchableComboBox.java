...
public class SearchableComboBox extends JComboBox {
	...
	private MouseListener[] comboButtonMouseListeners;
	...
	private void addKeyListenerToComboBox() {
		final Color colorDefault = ((JTextField)getEditor().getEditorComponent()).getForeground();
			    		...
			    		for (Component comp : getComponents()) {
							if(comp instanceof AbstractButton)
							{
								if(!comp.isEnabled()){
									...
									for (MouseListener mouseListener : comboButtonMouseListeners)
										comp.addMouseListener(mouseListener);
								}
							}
						}
			    		((JTextField)getEditor().getEditorComponent()).setForeground(colorDefault);
